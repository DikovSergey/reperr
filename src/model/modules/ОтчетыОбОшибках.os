...
Процедура ОбработатьОтчетОбОшибке(ФайлОтчета) Экспорт
	...
	// Удаляем старые распакованные отчеты из data по report.json.time (политика хранения)
	ХранилищеДанных.ОчиститьСтарыеОтчеты(ДанныеОтчетаОбОшибке.Идентификатор);

КонецПроцедуры

Функция ЗарегистрироватьОшибку(ДанныеОтчетаОбОшибке)

	ИдентификаторыТрекеров = Новый Структура;

	ПровайдерыИнтеграции = МенеджерНастроек.ПровайдерыИнтеграции();

	Если ПровайдерыИнтеграции <> Неопределено И ПровайдерыИнтеграции.Количество() > 0 Тогда
		Для Каждого Провайдер Из ПровайдерыИнтеграции Цикл
			Попытка
				ТекИд = Провайдер.ЗарегистрироватьОшибку(ДанныеОтчетаОбОшибке);
				ИмяПровайдера = СтрЗаменить(Строка(ТипЗнч(Провайдер)), "ПровайдерИнтеграции", "");
				Если ЗначениеЗаполнено(ТекИд) Тогда
					ИдентификаторыТрекеров.Вставить(ИмяПровайдера, ТекИд);
				Иначе
					ИдентификаторыТрекеров.Вставить(ИмяПровайдера, "registered");
				КонецЕсли;
			Исключение
				ТекстОшибки = СтрШаблон("Ошибка при регистрации в провайдере %1: %2", ТипЗнч(Провайдер), ОписаниеОшибки());
				Сообщить(ТекстОшибки);
				// Если хотя бы один не принял - пробрасываем ошибку дальше, чтобы 1С считала отчет неотправленным
				ВызватьИсключение ТекстОшибки;
			КонецПопытки;
		КонецЦикла;
	Иначе
		ВызватьИсключение("Регистрация ошибок в баг-трекере не настроена. Включите хотя бы одну интеграцию в appsettings.json: reperrSettings.Integrations (sentry, redmine, jira, rabbitMQ или elma365). Убедитесь, что приложение загружает конфиг из нужной папки (текущая или src/).");
	КонецЕсли;

	Возврат ИдентификаторыТрекеров;
		...
		Для Каждого ЭлементОшибки Из Ошибка Цикл
			...
			Если ТипЗнч(ЭлементОшибки) = Тип("Массив") Тогда
				Для Каждого ЭлементЭлементаОшибки Из ЭлементОшибки Цикл
					Если ЗначениеЗаполнено(ЭлементЭлементаОшибки) Тогда
						ЧастиОшибки.Добавить(ЭлементЭлементаОшибки);
					КонецЕсли;
				КонецЦикла;
			Иначе
				Если ЗначениеЗаполнено(ЭлементОшибки) Тогда
