	...
	Если ПровайдерыИнтеграции <> Неопределено И ПровайдерыИнтеграции.Количество() > 0 Тогда
		Для Каждого Провайдер Из ПровайдерыИнтеграции Цикл
			Попытка
				...
				Если ИдТрекера = Неопределено И ЗначениеЗаполнено(ТекИд) Тогда
					ИдТрекера = ТекИд; // Сохраняем ID от первого успешно отработавшего провайдера
				КонецЕсли;
			Исключение
				Сообщить(СтрШаблон("Ошибка при регистрации в провайдере %1: %2", ТипЗнч(Провайдер), ОписаниеОшибки()));
			КонецПопытки;
		КонецЦикла;
	Иначе
