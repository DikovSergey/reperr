...
Функция ЗарегистрироватьОшибку(ДанныеОтчетаОбОшибке)
	...
	Иначе
		ВызватьИсключение("Регистрация ошибок в баг-трекере не настроена. Включите хотя бы одну интеграцию в appsettings.json: reperrSettings.Integrations (sentry, redmine, jira или rabbitMQ). Убедитесь, что приложение загружает конфиг из нужной папки (текущая или src/).");
	КонецЕсли;
