...
// BSLLS:UnusedParameters-on
	...
	ВсеТесты.Добавить("ТестДолжен_ОчиститьСтарыеОтчетыПоПолюTime");
...
Процедура ТестДолжен_ЗаписатьОшибкуВФайл() Экспорт
	...
	ДанныеОтчетаПоОшибке.Вставить("fingerprint", "622C3BA24596958DD3A2D9AF0AAD16BC");
...
Процедура ТестДолжен_ОчиститьСтарыеОтчетыПоПолюTime() Экспорт
	
	Каталог = "data";
	ХранилищеДанных = Новый ФайловоеХранилище(Каталог);
	
	СтарыйИд = "old-report-id";
	НовыйИд = "new-report-id";
	
	КаталогСтарогоОтчета = ОбъединитьПути(Каталог, СтарыйИд);
	КаталогНовогоОтчета = ОбъединитьПути(Каталог, НовыйИд);
	
	ФС.ОбеспечитьКаталог(КаталогСтарогоОтчета);
	ФС.ОбеспечитьКаталог(КаталогНовогоОтчета);
	
	ЗаписатьReportJson(ОбъединитьПути(КаталогСтарогоОтчета, "report.json"), "2000-01-01T00:00:00Z");
	ЗаписатьReportJson(ОбъединитьПути(КаталогНовогоОтчета, "report.json"), "2999-01-01T00:00:00Z");
	
	НастройкаХранения = Новый Структура("enabled, hours", Истина, 1);
	ХранилищеДанных.НастроитьХранениеОтчетов(НастройкаХранения);
	ХранилищеДанных.ОчиститьСтарыеОтчеты();
	
	Ожидаем.Что((Новый Файл(КаталогСтарогоОтчета)).Существует()).Равно(Ложь);
	Ожидаем.Что((Новый Файл(КаталогНовогоОтчета)).Существует()).Равно(Истина);
	
КонецПроцедуры

Процедура ЗаписатьReportJson(ПутьКФайлу, Время)
	
	Текст = СтрШаблон("{""time"":""%1""}", Время);
